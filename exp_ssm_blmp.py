...
def get_blackbox_model(args):
    ...
    print("[CHK] torch.cuda.is_available():", torch.cuda.is_available(), "| args.device:", args.device)
    if torch.cuda.is_available():
        print("[CHK] cuda.count:", torch.cuda.device_count(), "| name:", torch.cuda.get_device_name(0))
    print("[CHK] blackbox_model device:", next(model.parameters()).device)
...
def train_visual_prompt_with_mapping(
    ...
    print("[CHK] visual_prompt device:", next(visual_prompt.parameters()).device)
    try:
        print("[CHK] network device:", next(network.parameters()).device)
    ...
    for epoch in range(epochs):
        ...
        for step, (x, y) in enumerate(pbar):
            if step == 0:
                print("[CHK] batch x BEFORE .to():", x.device)  # 一定是 CPU
            x = x.to(device, non_blocking=True); y = y.to(device, non_blocking=True)
            if step == 0:
                print("[CHK] batch x AFTER  .to():", x.device)   # 一定是 cuda:*
